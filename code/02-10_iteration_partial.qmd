...
```{r density plot} 
# Looking into density to explore data distribution 
# We do not provide y, just x. We have 41 cols and we want to plot for all.

nematode %>%
  pivot_longer(cols = c(mel_s:re)) %>%
  ggplot(aes(x = value)) +
...
While loop will work on a condition (we may not know the length)
...
for(i in 3:ncol(nematode_w)) {
  ...
  varname <- colnames[i] # Get every, single number on each itineration + 1.
  
  # Calculating mean and cv
  
  statsum <- meancv(nematode_w[[i]]) # This [[]] is to access the df as a vector.
...
```{r Very important to my project}
nematode_mods <- nematode_w %>%
  ...
  nest()%>%  # ALl of the informations are hold inside a new dataframe for each group
  # Applying linear model to each element
  mutate(lm = purrr::map(data,
    ~lm(resp_val ~ exp_val,
        data = .x) # This is to call the data that we already assigned before.
  )) %>%
...
nematode_mods_sel <- nematode_mods %>%
  unnest(r2) %>%
  group_by(resp_var) %>% #Group by the resp var that we need to see.
  slice_max(r2)
...
nematode_mods_sel_plots <- nematode_mods_sel %>%
 ...
 mutate (splot_better =pmap(list(.df = data,
                            ...
                            function(.df, .r2, .rv, .ev)
                              ...
                              geom_point() +
                              geom_smooth(method = "lm") +
                              labs(subtitle = paste("R2", round(.r2, 3)),
                                   x = .ev,
                                   y = .rv
                                   )
                            ))
nematode_mods_sel_plots$splot[2]
